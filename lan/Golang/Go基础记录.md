...
#### 函数的使用技巧
[[lan/Golang/Go的一些标准包/异常处理#使用 panic() defer recover() 实现异常的抛出与捕获]]

#### golang的指针类型

- **类型安全指针**，  是我们编码中最常用的指针， 用于传递对象地址， 不能进行指针运算
- **`unsafe.Pointer`** 通用指针类型，用于转换不同类型的指针，不能进行指针运算，不能读取内存存储的值(需转换到某一类型的普通指针)

- **`uintptr`**    用于指针运算， `uintptr`类型在golng内部不被视作指针， 他所指向的地址可能会被GC掉（不被视作活引用）。 
